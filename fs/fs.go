...
func New() (http.FileSystem, error) {
	if zipData == "" {
		return nil, errors.New("archiving/fs: no zip data registered")
	}
	...
	for _, zipFile := range zipReader.File {
		...
		if err != nil {
			return nil, fmt.Errorf("archiving/fs: error unzipping file %q: %s", zipFile.Name, err)
		}
		// files["/"+zipFile.Name] = f
		files[filepath.Clean("/"+zipFile.Name)] = f
	}
	for fn := range files {
